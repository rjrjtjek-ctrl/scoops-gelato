...
export default function StoreLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user.role !== "franchisee") { setError("점주 계정이 아닙니다."); return; }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FFF8F0] px-4">
      <div className="w-full max-w-[400px] bg-white rounded-2xl shadow-lg p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">🏪 점주 로그인</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={loginId} onChange={e => setLoginId(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-[#2C1810] text-sm" placeholder="점주 아이디" autoComplete="username" required />
          </div>
          <div>
          ...
          <button type="submit" disabled={loading} className="w-full py-3 rounded-lg text-white font-medium text-sm disabled:opacity-50" style={{ backgroundColor: "#1B4332" }}>
            {loading ? "로그인 중..." : "점주 로그인"}
          </button>
        </form>
      </div>
    </div>
